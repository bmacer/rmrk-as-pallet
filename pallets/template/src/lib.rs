...
pub mod pallet {
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
		...
		/// Identifier for the class of asset.
		type RmrkCollectionId: Member + Parameter + Default + Copy + HasCompact;
	...
	pub(super) type RmrkCollection<T: Config<I>, I: 'static = ()> =
		StorageMap<_, Blake2_128Concat, T::RmrkCollectionId, RmrkCollectionDetails<T::AccountId>>;
	...
	pub(super) type InstanceRmrkCollectionMetadataOf<T: Config<I>, I: 'static = ()> = StorageMap<
		_,
		Blake2_128Concat,
		T::RmrkCollectionId,
		InstanceRmrkMetadata<T::StringLimit>,
		OptionQuery,
	>;
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
		...
		/// Rmrk Collection Created.
		RmrkCollectionCreated {
	...
	#[pallet::call]
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		/// Issue a new RMRK collection
		#[pallet::weight(T::WeightInfo::create())]
		pub fn create_rmrk_collection(
			origin: OriginFor<T>,
			...
			Self::do_create_rmrk_collection(
				class,
				owner.clone(),
				max,
				symbol.to_vec(),
				...
				// T::ClassDeposit::get(),
				// false,
				Event::RmrkCollectionCreated { class, creator: owner.clone(), owner },
			)
		}
		...
		pub fn changeissuer(
			origin: OriginFor<T>,
			// #[pallet::compact] rmrk_class: T::ClassId,
			#[pallet::compact] rmrk_class: T::RmrkCollectionId,
			issuer: <T::Lookup as StaticLookup>::Source,
			// admin: <T::Lookup as StaticLookup>::Source,
			// freezer: <T::Lookup as StaticLookup>::Source,
		) -> DispatchResult {
			...
			RmrkCollection::<T, I>::try_mutate(rmrk_class, |maybe_details| {
				let details = maybe_details.as_mut().ok_or(Error::<T, I>::Unknown)?;
				ensure!(&origin == &details.issuer, Error::<T, I>::NoPermission);
		...
		///
		/// Weight: `O(1)`
		#[pallet::weight(T::WeightInfo::set_rmrk_collection_metadata())]
		pub fn set_rmrk_collection_metadata(
			origin: OriginFor<T>,
			#[pallet::compact] class: T::RmrkCollectionId,
			// #[pallet::compact] instance: T::InstanceId,
			data: BoundedVec<u8, T::StringLimit>,
			...
			let mut class_details =
				RmrkCollection::<T, I>::get(&class).ok_or(Error::<T, I>::Unknown)?;

			if let Some(check_owner) = &maybe_check_owner {
				ensure!(check_owner == &class_details.issuer, Error::<T, I>::NoPermission);
			}

			InstanceRmrkCollectionMetadataOf::<T, I>::try_mutate_exists(class, |metadata| {
				...
				*metadata = Some(InstanceRmrkMetadata { data: data.clone() });

				RmrkCollection::<T, I>::insert(&class, &class_details);
