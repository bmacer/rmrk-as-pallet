...
#[test]
fn set_rmrk_collection_metadata_should_work() {
	new_test_ext().execute_with(|| {
		println!("set_rmrk_collection_metadata");
		let a = decode_hex("7b226e616d65223a226272616e646f6e227d");
		println!("a: {:?}", a);

		let y = vec![1, 2, 3];
		let z = BoundedVec::<u8, StringLimit>::try_from(y).unwrap();
		// let x = bvec![1, 2, 3, 4];
		...
		let t = Uniques::set_rmrk_collection_metadata(
			Origin::signed(0),
			0,
			"2537422530442".as_bytes().to_vec().try_into().unwrap(), // "abc".as_bytes().to_vec().try_into().unwrap(), // .try_into().unwrap(), // "hello".as_bytes().try_into().unwrap(), // BoundedVec < "0x7b226e616d65223a226272616e646f6e227d",
		);
...
fn changeissuer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Uniques::changeissuer(Origin::signed(0), 0, 1));

		println!("{:?}", Uniques::get_rmrk_collection(0));

		assert_ok!(Uniques::changeissuer(Origin::signed(1), 0, 2));

		println!("{:?}", Uniques::get_rmrk_collection(0));

		assert_noop!(Uniques::changeissuer(Origin::signed(0), 0, 1), Error::<Test>::NoPermission);
